...
class CMAES:
  ...
  def __init__(self, num_params,      # number of model parameters
    ...
    if initial_weights is None:
      initial_weights = self.num_params * [0]
    
    self.es = cma.CMAEvolutionStrategy( initial_weights,
                                        self.sigma_init,
...
class OpenES:
  ...
  def __init__(self, num_params,             # number of model parameters
    ...
    if initial_weights is None:
      initial_weights = np.zeros(self.num_params)

    self.mu = np.array(initial_weights)
