...
class Smart_Algorithm3 : public Base {
   private:
    ...
    void get_heuristic_based_icbp_list(std::vector<vector_3d> &heuristic_based_icpbcp_list, std::vector<std::vector<int>> &state, vector_3d dim) {
        ...
        for (int i = 0; i < BIN_WIDTH; i++)
        {
            for (int j = 0; j < BIN_LENGTH; j++)
            {
                ...
                if(!first_fit_flag1 && flag){
                    heuristic_based_icpbcp_list.push_back({i,j,0});
                    first_fit_flag1=1;
                }
                double s;
                if (flag && min_height1 > (s=score(state, {i, j}, {lx, ly, lz})))
                {
                    pos_orientation1 = {i, j, 0};
                    min_height1=s;
                }
                
                if(flag && walleScoreMax1<(s=calculate_walle_score(state, {i,j}, {lx, ly, lz}))){
                    walleScoreMax1=s;
                    walle_pos_ori1.clear();
                    walle_pos_ori1.push_back({i,j,0});
                }
                if(flag && walleScoreMax1==s){
                    walle_pos_ori1.push_back({i,j,0});
                }


                flag = check_with_precomputation(state, {i, j, 0}, {ly, lx, lz});
                if(!first_fit_flag2 && flag){
                    heuristic_based_icpbcp_list.push_back({i,j,1});
                    first_fit_flag2=1;
                }
                if (flag && min_height2 > (s=score(state, {i, j}, {ly, lx, lz})))
                {
                    pos_orientation2 = {i, j, 1};
                    min_height2=s;
                }
                if(flag && walleScoreMax2<(s=calculate_walle_score(state, {i,j}, {ly, lx, lz}))){
                    walleScoreMax2=s;
                    walle_pos_ori2.clear();
                    walle_pos_ori2.push_back({i,j,1});
                }
                if(flag && walleScoreMax2==s){
                    walle_pos_ori2.push_back({i,j,1});
                }
            }
        }
   ...
   public:
    ...
    Smart_Algorithm3(GenerateBox gb, Sim &simulator, const std::vector<double> &params) : Base(gb, simulator) {
        this->params = params;
    }
