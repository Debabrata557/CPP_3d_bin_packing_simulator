...
if __name__ == '__main__':
    ...
    BIAS_HOLE=10
    TOTAL_PARAMS = BOUNDARY_PARAMS + BIAS_HOLE+POOL_PARAMS
    ...
    def run_solver(solver):
        ...
        for j in range(MAXITER):
            ...
            solver.tell([x*100000-50000 for x in effs])
            ...
            print('train result: ')
            print(row)
            curr_weights='cur_weights.txt'
            if np.mean(effs)>bestTrainMean:
                bestTrainMean=np.mean(effs)
                save_weights_to_file(solver.current_param().tolist(),f'{directory}/{curr_weights}')
            if(j%300==299):
    ...
    def run_simulation(seed,episode,algo_name, weights=None, gen=None,test=False):
        ...
        boxType="random"
        openBinCount=1
        no_of_random_boxes=80
        if test:
            openBinCount=4
            no_of_random_boxes=300
        return run_subproccess(['./build/run',filename_in,filename_out,str(seed),str(episode), algo_name, boxType, str(openBinCount), str(debug), str(no_of_random_boxes)])
